...
const ResetPassword = () => {
  ...
  const onSubmitEmail=async(e)=>{
    e.preventDefault();
    try {
       const { data } = await axios.post(backendUrl + "/api/auth/send-password-reset-otp", {
         email
        });
    ...
    <div className=" flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-300 to-purple-500">
      ...
      {!isEmailSent && (
        ...
        className=" bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm">
          ...
          <button className=" w-full py-2.5  bg-gradient-to-r from-indigo-500 to-indigo-900 rounded-full text-white cursor-pointer mt-3">
            Verify Email
          </button>
        </form>
      )}
            ...
            {Array(6)
              ...
              .map((_, index) => (
                <input
                  ...
                  key={index}
                  required
                  className=" w-12 h-12 bg-[#333A5C]  text-white text-center text-xl rounded-md no-arrow"
